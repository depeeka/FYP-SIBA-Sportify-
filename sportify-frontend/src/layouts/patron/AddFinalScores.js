                        ...
                        {JSON.parse(sessionStorage.getItem("login")) && JSON.parse(sessionStorage.getItem("login")).role !== 'PLAYER' ?
                            <form onSubmit={handleAdd}>
                                ...
                                <Select
                                    placeholder='Status'
                                    value={score.status}
                                    onChange={(value) => setScore({...score, status: value})}
                                    className={classes.textField}
                                    style={{width:'300'}}>
                                    <Select.Option value='' disabled>Select Status</Select.Option>
                                    {WinStatus.length > 0 ? (
                                        WinStatus.map((venue, index) => (
                                            <Select.Option key={`${venue.status}-${index}`} value={`${venue.status}`}>
                                                {`${venue.status}`}
                                            </Select.Option>
                                        ))
                                    ) : (
